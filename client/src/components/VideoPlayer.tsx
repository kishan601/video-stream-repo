        ...
        {!showLoadingText && (
          <div className="absolute top-3 right-3">
            {(isLoading || hasError) ? (
              ...
              <Badge 
                className="flex items-center gap-1.5 px-3 py-1 bg-cyan-500 text-black hover:bg-cyan-400 shadow-glow-cyan"
                data-testid={`badge-live-${stream.id}`}
              >
                <span className="relative flex h-2 w-2">
                  <span className="animate-pulse absolute inline-flex h-full w-full rounded-full bg-cyan-300 opacity-100"></span>
